...
proc new_orca { name args } {
  ...
  set sloppyHS undefined
  set sloppyTol undefined 
...
#RB. Simple way to check if doing first cycle or not and if doing sloppyHS 
set sloppyHSflag undefined
if { ! $restart_orca } {
  if { $sloppyHS == "yes" } {
    set sloppyHSflag yes
  }
}

# "Simple input". Adding Engrad keyword if gradient-job (unless sloppyHSflag)
if { $gradflag } then {
    if {$sloppyHSflag == "yes"} {
      puts $fp "$orcasimpleinput"
    } else {
      puts $fp " $orcasimpleinput Engrad"
    }
} else { 
  puts $fp "$orcasimpleinput" 
...
#Adding options to allow for ORCA Flipspin-BS optimization
  ...
  if { $restart_orca } {
    ...
    # Case: 1st Engrad step. Adding possible Flipspin/FinalMS keywords here
    if { $brokensym == "yes" } {
      #If sloppyHS is set then we create multijob inputfile where first job is sloppy HS calc
      if { $sloppyHS == "yes" } {
        #set sloppyHSflag yes
        puts $fp "TolE $sloppyTol"
        puts $fp "ConvCheckMode 1"
        set mult $hsmult
      } else {
...
#####################
# SPECIAL SLOPPYHS PROCEDURE to save time in big BrokenSym jobs
# Will make an inputfile for an ORCA multijob where Sloppy tolerances (defined above) is applied to the 1st HS job.
# The second job then reads the sloppy HS orbitals, flips immediately and converges to BS solution.  
#######################
#:# Colors Normal: \033\[0m   Green: \033\[32m    Red: \033\[31m  Yellow: \033\[33m Blue: \033\[34m   Magenta: \033\[35m    Cyan: \033\[36m
  #If doing 1st BS cycle and doing sloppyHS then we add a new BS job at bootm
  if { $sloppyHSflag == "yes" } {
   puts ""
    puts "\033\[33mSloppyHS settings found. 1st Chemshell Opt cycle will do the following:"
    puts "\033\[32m1. Run multi-job ORCA inputfile (\$new_job feature) where Sloppy tolerances (TolE $sloppyTol) are applied to HS solution."
    puts "\033\[32m2. Sloppy HS orbitals read in, spins flipped and regular convergence to BS solution"
    puts "\033\[32mIn next Opt cycles, the BS-calculation reads in old orbitals (from previous job) as normal.\033\[0m"
    puts ""
    puts $fp ""
    puts $fp "\$new_job"
    puts $fp ""
...
#Adding options to allow for ORCA Flipspin-BS optimization
  ...
  # SCF block to handle Autostart and guess orbitals
  puts $fp "%scf"
  #Setting TolE to 1000 so that the HS calc from previous job is not touched. Will not apply to BS solution
  puts $fp "TolE 1000"
  puts $fp "ConvCheckMode 1"
  puts $fp "Flipspin $spinstoflip"
  puts $fp "FinalMS [expr ($bsmult - 1 ) / 2.0 ]"
...
if {$sloppyHSflag == "yes"} {
puts "\033\[33mSloppy-procedure in first Opt cycle done. From now on doing regular BS-state calculations with MOREAD. \033\[0m"
}
  #
  # ======================= Read ORCA output =======================
  #
